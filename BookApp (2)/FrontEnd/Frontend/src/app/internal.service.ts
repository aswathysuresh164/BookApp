...
getFlag()
{
  ...
  getFavouriteBooks(username): Observable<Array<book>> {
    return this.httpClient.get<Array<book>>(`http://localhost:8089/api/bookApp/getFavourites/${username}`);
  }
  deleteFromFavourites(id,username):Observable<book> {
   
    return this.httpClient.delete<book>(`http://localhost:8089/api/bookApp/deleteFromFavourite/${id}/${username}`);
  }
  ...
  getFavouriteBooksByIdAndUsername(id,username):Observable<book> {
    ...
    return this.httpClient.get<book>(`http://localhost:8089/api/bookApp/getFavouriteBooksByIdAndUsername/${id}/${username}`);
  }
...
addBookToFav(book:book): Observable<book>{
  return this.httpClient.post<book>(`http://localhost:8089/api/bookApp/addToFavourites`,book);
}
